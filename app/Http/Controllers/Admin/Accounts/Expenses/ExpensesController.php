...
namespace App\Http\Controllers\Admin\Accounts\Expenses;
...
class ExpensesController extends Controller
{
    public function index(){
        $data['assets']=Account::query()->with('balanceTypes')->where('type_id',2)->get();
        $data['balanceTypes']=Constant::query()->where('category','like','balance_types')->get();

        return view('admin.accounts.expenses.index',$data);
    }

    public function getIndex(Request $request){
        $accounts = Account::query()->with('balanceTypes')->where('type_id',2)
        ...
        return datatables()->of($accounts)
            ...
            ->addColumn('action', function ($data) {
                ...
                return view('admin.accounts.expenses.partials.actions',compact('data'));
            })
            ->rawColumns([ 'action'])
            ->make(true);

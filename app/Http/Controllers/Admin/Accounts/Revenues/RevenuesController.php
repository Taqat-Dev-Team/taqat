...
namespace App\Http\Controllers\Admin\Accounts\Revenues;
...
class RevenuesController extends Controller
{
    public function index(){
        $data['assets']=Account::query()->with('balanceTypes')->where('type_id',3)->get();
        $data['balanceTypes']=Constant::query()->where('category','like','balance_types')->get();

        return view('admin.accounts.revenues.index',$data);
    }

    public function getIndex(Request $request){
        $accounts = Account::query()->with('balanceTypes')->where('type_id',3)
        ...
        return datatables()->of($accounts)
            ...
            ->addColumn('parnent', function ($account) {
            ...
            $button .= '&nbsp;&nbsp;&nbsp;&nbsp;';
            // if(auth('admin')->user()->can('edit_account')){
                $button .= '<a  class="edit" data-account_id="' . $account->id . '" data-parent_id="'.$account->parent_id.'" data-name="'.$account->name.'"
                data-balance_type_id="'.$account->balance_type_id.'"
                ><span><i style="color: blue" class="fas fa-edit"></i></span></a>';
            // }
            $button .= '&nbsp;&nbsp;&nbsp;&nbsp;';
            // if(auth('admin')->user()->can('delete_account')){
                $button .= '<a id="' . $account->id . '" name_delete="' . $account->name . '" class="delete"><span><i style="color: red" class="fa fa-trash"></i></span></a>';
            // }
            return $button;
            })
            ->rawColumns([ 'action'])

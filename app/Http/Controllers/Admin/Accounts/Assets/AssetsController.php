...
namespace App\Http\Controllers\Admin\Accounts\Assets;
...
class AssetsController extends Controller
{
    public function index(){
        $data['assets']=Account::query()->with('balanceTypes')
        ->where('type_id',1)->get();
        $data['balanceTypes']=Constant::query()->where('category','like','balance_types')->get();

        return view('admin.accounts.assets.index',$data);
    }

    public function getIndex(Request $request){
        $data = Account::query()->with('balanceTypes')->where('type_id',1)->whereNotnull('parent_id');
        ...
        return datatables()->of($data)
            ...
            ->addColumn('action', function ($data) {
                ...
                return view('admin.accounts.assets.partials.actions',compact('data'));
            })
            ->rawColumns([ 'action'])
            ->make(true);

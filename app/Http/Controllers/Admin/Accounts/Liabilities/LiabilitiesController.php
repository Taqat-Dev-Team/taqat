...
namespace App\Http\Controllers\Admin\Accounts\Liabilities;
...
class LiabilitiesController extends Controller
{
    public function index(){
        $data['liabilities']=Account::query()->with('balanceTypes')->where('type_id',4)->get();
        $data['balanceTypes']=Constant::query()->where('category','like','balance_types')->get();

        return view('admin.accounts.liabilities.index',$data);
    }

    public function getIndex(Request $request){
        $data = Account::query()->with('balanceTypes')->where('type_id',4)
        ...
        return datatables()->of($data)
            ...
            return view('admin.accounts.liabilities.partials.actions',compact('data'));

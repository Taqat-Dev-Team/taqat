        ...
        <div class="d-flex flex-center flex-column flex-column-fluid p-10 pb-lg-20">
            ...
            <div class="w-lg-600px bg-body rounded shadow-sm p-10 p-lg-15 mx-auto">
                ...
                <div  class="form-container active">
                    <form class="form w-100"  name="login-form" id="login-form" method="post" action="{{route('companies.login.postlogin')}}">
                        @csrf
                        <div class="text-center mb-10">
                            <h1 class="text-dark mb-3">{{__('label.login')}}</h1>
                            <div class="text-gray-400 fw-bold fs-4">

                                <a href="{{route('companies.register')}}" class="link-primary fw-bolder">{{__('label.Create_an_Account')}}</a>
                            </div>
                        </div>
                        <!--begin::Input group-->
    ...
    document.getElementById('language-toggle').addEventListener('click', function (event) {
        ...
        $(document).ready(function() {
            $('#login-form').submit(function(e) {
                ...
                $.ajax({
                    url: "{{ route('companies.login.postlogin') }}", // Ensure this route is correct
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function(response) {
                        ...
                        window.location.href = "{{route('companies.index')}}"; // Adjust the redirection if needed
                    },
                    error: function(xhr) {

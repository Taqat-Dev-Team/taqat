...
@section('content')
    <div class="card card-custom">
        <div class="card-header flex-wrap">
            <div class="card-title">
                <h1 class="card-label">{{ __('label.list_generator_receipts') }}</h1>
            </div>

            <div class="card-toolbar">
                @if (auth('admin')->user()->can('add_generator_receipt') && !$status)
                    <a href="#" class="btn btn-success mr-1" id="add_edit">
                        <i class="fa fa-plus"></i>
                        {{ __('label.add_generator_receipt') }}
                    </a>
                @endif
            ...
            <div class="row">
                ...
                <div class="col-lg-6 col-md-6 col-sm-12">
                    <div class="card card-custom bg-danger text-white shadow-sm">
                        <div class="card-body text-center">
                            <h5 class="card-title font-weight-bold">
                                <i class="fa fa-exclamation-circle mr-2"></i>{{ __('label.total_collections') }}
                            </h5>
                            <p class="card-text display-4 font-weight-bold" id="total_collections"></p>
                        </div>
                    </div>
                </div>
        ...
        @include('admin.generatorReceipts.modal.add_edit')
        @include('Shared.delete')


        <input name="status" id="add_edit_status" value="{{$status}}" type="hidden"  >
    ...
    @section('scripts')
        @include('admin.generatorReceipts.js.js')
    @endsection

...
<script>
    $(document).ready(function() {
        ...
        $('#kt_modal_add_edit').on('hidden.bs.modal', function() {
            ...
            $('.error').text('');
            $('#my-form')[0].reset();
            $('.modal-title').text("{{ __('messages.add_new_liability') }}");
            // Set the action attribute of the form
            form.attr('action', "{{ route('admin.accounts.equities.store') }}");
        ...
        let table = $('#data-table').DataTable({
            ...
            ajax: {
                url: "{{ route('admin.accounts.equities.getIndex') }}",
        ...
        $(document).on('click', '.view, .edit', function(e) {
            ...
            const title = $(this).hasClass('view') ? '{{ __('messages.view_equities') }}' :
                '{{ __('messages.edit_equities') }}';

            const form = $('#my-form'); // The form you want to set the action for

            // Set the action attribute of the form
            form.attr('action', "{{ route('admin.accounts.equities.update') }}");

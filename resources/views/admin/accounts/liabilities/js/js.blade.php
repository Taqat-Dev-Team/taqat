...
<script>
    $(document).ready(function() {
        ...
        $('#kt_modal_add_edit').on('hidden.bs.modal', function() {
            ...
            $('.error').text('');
            $('#my-form')[0].reset();
            $('.modal-title').text("{{ __('label.add_new_liability') }}");
            // Set the action attribute of the form
            form.attr('action', "{{ route('admin.accounts.liabilities.store') }}");
            $('#add_edit_b').val('').trigger('change.select2');
        ...
        let table = $('#data-table').DataTable({
            ...
            ajax: {
                url: "{{ route('admin.accounts.liabilities.getIndex') }}",
        ...
        $(document).on('click', '.view, .edit', function(e) {
            ...
            const title = $(this).hasClass('view') ? '{{ __('messages.view_liabilities') }}' :
                '{{ __('label.edit_liabilities') }}';

            const form = $('#my-form'); // The form you want to set the action for

            // Set the action attribute of the form
            form.attr('action', "{{ route('admin.accounts.liabilities.update') }}");
            ...
            $('#' + action + '_parent_id ').val($(this).data('parent_id')).trigger('change');
    ...
    $(document).on('click', '.submit', function(e) {
        ...
        $.ajax({
            url: '{{ route('admin.accounts.liabilities.delete') }}',
            method: 'POST',
            data: {
                "id": ids,
                "_token": "{{ csrf_token() }}",

...
<script>
    $(document).ready(function() {
        ...
        $('#kt_modal_add_edit').on('hidden.bs.modal', function() {
            ...
            $('.error').text('');
            $('#my-form')[0].reset();
            $('.modal-title').text("{{ __('label.add_new_expenses') }}");
            // Set the action attribute of the form
            form.attr('action', "{{ route('admin.accounts.expenses.store') }}");
        ...
        let table = $('#data-table').DataTable({
            ...
            ajax: {
                url: "{{ route('admin.accounts.expenses.getIndex') }}",
        ...
        $(document).on('click', '.view, .edit', function(e) {
            ...
            const title = $(this).hasClass('view') ? '{{ __('label.view_expense') }}' :
                '{{ __('label.edit_expense') }}';

            const form = $('#my-form'); // The form you want to set the action for

            // Set the action attribute of the form
            form.attr('action', "{{ route('admin.accounts.expenses.update') }}");
    ...
    $(document).on('click', '.submit', function(e) {
        ...
        $.ajax({
            url: '{{ route('admin.accounts.expenses.delete') }}',
            method: 'POST',
            data: {
                "id": ids,
                "_token": "{{ csrf_token() }}",

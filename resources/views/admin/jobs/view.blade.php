    ...
    <div class="card card-custom">
        <div class="card-header flex-wrap">
            <div class="card-title">
                <h1 class="card-label">{{ __('label.show_job') }}</h1>
            </div>
                            ...
                            <div class="tab-content pt-5">
                                ...
                                <div class="tab-pane active" id="kt_apps_contacts_view_tab_1" role="tabpanel">
                                    <div class="container">
                                        ...
                                        <span
                                            class="font-weight-bolder mb-4">{{ __('label.count_apply_requests') }}</span>
                                        <span class="font-weight-bolder font-size-h5 pt-1">
                                            <span
                                                class="font-weight-bold text-dark-50">({{ $job->applyJobCount() }})</span>
                                            ...
                                            <span class="font-weight-bolder font-size-h5 pt-1">
                                                ...
                                                <span class="font-weight-bolder font-size-h5 pt-1">
                                                    ...
                                                    <span
                                                        class="font-weight-bolder mb-4">{{ __('label.permanent_type') }}</span>
                                                    <span class="font-weight-bolder font-size-h5 pt-1">
                                                        <span
                                                            class="font-weight-bold text-dark-50">{{ $job->permanent_type }}</span>
                                                        <div class="separator separator-dashed my-10"></div>
                                                        <span
                                                            class="font-weight-bolder mb-4">{{ __('label.status') }}</span>
                                                        <span class="font-weight-bolder font-size-h5 pt-1">
...
@section('scripts')
    ...
    <script>
        $('.delete').on('click', function(e) {
            e.preventDefault();
            var project_id = $(this).data('delete_id');

            Swal.fire({
                title: '{{ __('label.are_you_sure') }}',
                text: "{{ __('label.are_yout_sure_delete') }}",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        data: {
                            _token: '{{ csrf_token() }}',
                            'id': project_id // Laravel CSRF token
                        },
                        success: function(response) {
                            Swal.fire(
                                'Deleted!',
                                '{{ __('label.delete_elemnet_success_full') }}',
                                'success'
                            ).then(() => {
                                window.location.href =
                                    '{{ route('companies.jobs.index') }}'; // Redirect URL
                            });
        ...
        $(document).ready(function() {
            $('.stars').on('mouseover', '.star', function() {
                var $star = $(this);
                var ratingType = $star.closest('.stars').data('rating-type');
                var ratingValue = $star.data('value');

                // Highlight stars on hover
                $star.closest('.stars').find('.star').each(function() {
                    $(this).toggleClass('filled', $(this).data('value') <= ratingValue);
                });
            });

            $('.stars').on('mouseleave', function() {
                var ratingType = $(this).data('rating-type');
                var currentRating = $('#' + ratingType).val();
        ...
        $('#rating-form').on('click', function(e) {
            e.preventDefault(); // Prevent the default form submission

            var professional_dealing = $('#professional_dealing').val();
            var communication_assistance = $('#communication_assistance').val();
            var experience_in_project_field = $('#experience_in_project_field').val();
            var quality_delivered_work = $('#quality_delivered_work').val();
            var delivery_on_time = $('#delivery_on_time').val();
            var project_id = $('#project_id').val();
            var deal_with_again = $('#deal_with_again').val();
            ...
            $.ajax({
                url: '{{ route('companies.projects.submitRating') }}', // Adjust route name as needed
                type: 'POST',
                data: {
                    'professional_dealing': professional_dealing,
                    'communication_assistance': communication_assistance,
                    'quality_delivered_work': quality_delivered_work,
                    'experience_in_project_field': experience_in_project_field,
                    'delivery_on_time': delivery_on_time,
                    'message': message,
                    'project_id': project_id,
                    'deal_with_again': deal_with_again,
        ...
        //     var result = confirm("هل تريد انتقال الى مرحلة التنفيد المشروع");
        //     if (result) {
        //         $.ajax({

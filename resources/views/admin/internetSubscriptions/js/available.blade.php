...
<script>
    $(document).ready(function() {
        ...
        let table = $('.data-table').DataTable({
            ...
            searching: true,
            ajax: {
                url: "{{ route('admin.internetSubscriptions.getAvailable') }}",
                type: 'get',
                "data": function(d) {
        ...
        $(document).ready(function() {
            ...
            $("#searchUserButton").on("click", function() {
                ...
                if (mobile.length > 0) {
                    ...
                    toastr.warning('{{ __('messages.Please enter a valid mobile number') }}',
                        'Warning', {
                            timeOut: 3000
                        });
                }
            ...
            //     if ($(this).val() == '1') {
            //         // Show payment method options if status is "مدفوع"
            //         $('.form-check').show();
            //     } else {
            //         // Hide payment method options if status is "غير مدفوع"
            //         $('.form-check').hide();
        ...
        $(document).ready(function() {
            function toggleInvoiceData() {
                const createInvoice = $('#add_edit_create_invoice').val();
                if (createInvoice == '1') {
                    $('.invoice_data').show();
                } else {
                    $('.invoice_data').hide();
            ...
            function togglePaymentMethod() {
                const paymentStatus = $('#paymentStatus_exist').val();
                if (paymentStatus == '1') {
                    // مدفوع
                    $('input[name="payment_method"]').closest('.form-group').show();
                } else {
                    // غير مدفوع
                    $('input[name="payment_method"]').closest('.form-group').hide();
                }
            }

            // عند تغيير خيار انشاء فاتورة
            $('#add_edit_create_invoice').on('change', function() {
                toggleInvoiceData();
                togglePaymentMethod(); // تأكد من إعادة التحقق من حالة الدفع
            });

            // عند تغيير حالة الدفع
            $('#paymentStatus_exist').on('change', function() {
                togglePaymentMethod();
            });

            // تشغيل أولي عند فتح الصفحة أو المودال
            toggleInvoiceData();
            togglePaymentMethod();
        });

...
<script>
    $(document).ready(function() {
        ...
        $('#kt_modal_add_edit').on('hidden.bs.modal', function() {
            ...
            form.attr('action', "{{ route('admin.workSpaceManagments.services.store') }}");
        ...
        let table = $('.data-table').DataTable({
            ...
            ajax: {
                url: "{{ route('admin.workSpaceManagments.services.getIndex') }}",
        ...
        $(document).on('click', '.view, .edit', function(e) {
            ...
            const form = $('#my-form');
            form.attr('action', "{{ route('admin.workSpaceManagments.services.update') }}");

            // قائمة الحقول التي سيتم تعبئتها
            const fields = ['name', 'amount', 'service_id', 'currency_cd_id', 'is_monthly'];
    ...
    $(document).on('click', '.submit', function(e) {
        ...
        $.ajax({
            url: '{{ route('admin.workSpaceManagments.services.delete') }}',
            method: 'POST',
            data: {
                "id": ids,
                "_token": "{{ csrf_token() }}",

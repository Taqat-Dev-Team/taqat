...
<script>
    $(document).ready(function() {
        ...
        $('#kt_modal_add_edit').on('hidden.bs.modal', function() {
            ...
            form.attr('action', "{{ route('restaurants.products.store') }}");
        ...
        var table = $('.data-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('restaurants.products.getIndex') }}", // Correcting quotes around URL
                data: function(q) {
                    q.restaurant_id = "{{auth('restaurant')->check()}}"
    ...
    $(document).on('click', '.view, .edit', function() {
        ...
        // Set the action attribute of the form
        form.attr('action', "{{ route('restaurants.products.update') }}");

        const fields = [
            'category_id', 'name', 'logo', 'is_active',  'price', 'product_id'
        ];
    ...
    $(document).on('click', '.submit_delete', function(e) {
        ...
        $.ajax({
            url: '{{ route('restaurants.products.delete') }}',
            method: 'POST',
            data: {
                "id": ids,
                "_token": "{{ csrf_token() }}"
    ...
    function toggleActive(element) {
        ...
        $.ajax({
            url: '{{ route('restaurants.products.updateStatus') }}', // Laravel route for toggling the status
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
